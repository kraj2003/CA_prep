...
import {
  ...
  FileUp, Settings2, ArrowRight, Flame, Clock,
...
const TWEAK_PRESETS = [
  { label: "More MCQs", value: "Generate 10 scenario-based MCQs with harder difficulty" },
  { label: "Concise", value: "Make revision notes shorter, more bullet-focused for last-day revision" },
  { label: "Numericals", value: "Add more numerical examples and solved illustrations" },
  { label: "Exam Focus", value: "Focus only on most frequently tested aspects, ignore peripheral content" },
];
...
function DiffBadge({ level }: { level?: string }) {
  ...
  return (
    <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded border tracking-wide uppercase", map[key] ?? map.Medium)}>
      {key}
    </span>
  );
}
...
function MarkRevisedBtn({ sectionKey }: { sectionKey: string }) {
  const [done, setDone] = useState(false);
  return (
    <button
      onClick={() => setDone(p => !p)}
...
function SectionShell({
  ...
  const colors: Record<string, { icon: string; badge: string; border: string; bg: string }> = {
  ...
  return (
    <div
      className={cn(
        ...
        open ? cn(c.border) : "border-white/[0.06]"
      )}
    ...
    >
      ...
      <button
        ...
        className={cn(
          ...
          open ? cn(c.bg) : "hover:bg-white/[0.02]"
        )}
      >
        <div className={cn("w-8 h-8 rounded-xl flex items-center justify-center shrink-0", open ? c.bg : "bg-white/[0.04]", "border", open ? c.border : "border-white/[0.06]")}>
        ...
        <div className="flex items-center gap-2.5 flex-1 min-w-0">
          ...
          <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded border tracking-wider uppercase shrink-0", open ? c.badge : "bg-white/[0.04] text-white/25 border-white/[0.06]")}>
            {badge}
          </span>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {open && (
            <>
              <MarkRevisedBtn sectionKey={sectionKey} />
              <CopyBtn text={copyText} />
            </>
          )}
          <div className={cn("w-6 h-6 rounded-lg flex items-center justify-center border", open ? cn(c.badge) : "bg-white/[0.04] border-white/[0.06]")}>
            {open
  ...
  return (
    <div className="space-y-1">
      {text.split("\n").map((line, i) => {
        ...
        if (line.startsWith("## ")) {
          return (
            <h3 key={i} className="text-xs font-bold text-blue-300/80 uppercase tracking-widest pt-4 pb-1 first:pt-0"
        ...
        if (line.startsWith("# ")) {
          return (
            <h2 key={i} className="text-sm font-bold text-white pt-3 pb-1 first:pt-0">{line.slice(2)}</h2>
          );
        }
        if (line.match(/^[•\-]\s/)) {
          return (
            <div key={i} className="flex gap-2.5 items-start py-0.5">
              ...
              <p className="text-sm text-white/65 leading-relaxed flex-1"
                dangerouslySetInnerHTML={{ __html: render(line.slice(2)) }} />
            </div>
          );
        }
        if (/^\d+\.\s/.test(line)) {
          ...
          if (m) return (
            <div key={i} className="flex gap-2.5 items-start py-0.5">
              <span className="text-[11px] font-mono font-bold text-blue-400/60 shrink-0 mt-0.5 w-4">{m[1]}.</span>
              <p className="text-sm text-white/65 leading-relaxed flex-1"
                dangerouslySetInnerHTML={{ __html: render(m[2]) }} />
            </div>
          );
        }
    ...
    <div className="space-y-3">
      {sections.map((sec, i) => (
        <div key={i} className="rounded-xl border border-white/[0.06] overflow-hidden">
          {sec.heading && (
            <div className="px-4 py-2.5 bg-blue-500/[0.06] border-b border-white/[0.06]">
              <p className="text-[11px] font-bold text-blue-300/80 uppercase tracking-widest">
                {sec.heading}
              </p>
            </div>
          )}
          <div className="px-4 py-3 bg-white/[0.01]">
            <MD text={sec.body.trim()} />
          </div>
        </div>
      ))}
    </div>
        ...
        return (
          <div key={i} className="rounded-xl border border-white/[0.06] overflow-hidden">
            ...
            <div className="flex items-start gap-3 px-4 py-3.5 bg-white/[0.02]">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {q.examTip && (
                    <span className="text-[10px] text-white/25 truncate">💡 {q.examTip}</span>
                  )}
                </div>
                <p className="text-sm text-white/85 leading-relaxed font-medium">{q.question}</p>
              </div>
            </div>
              ...
              {q.options.map((opt, j) => {
                ...
                return (
                  <div
                    key={j}
                    className={cn(
                      "flex items-start gap-2.5 rounded-lg border px-3 py-2.5 text-sm transition-all",
                      isCorrect
                        ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-300"
                        : "bg-white/[0.02] border-white/[0.06] text-white/55"
                    )}
                  >
                    <span className={cn("font-bold shrink-0 text-xs mt-0.5", isCorrect ? "text-emerald-400" : "text-white/30")}>
                      {letter}.
                    </span>
            ...
            <button
              ...
              className="w-full text-center text-xs font-semibold py-2 border-t border-white/[0.04] transition-colors text-white/30 hover:text-white/60 hover:bg-white/[0.02]"
            >
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                  ...
                  <span className="text-[10px] text-white/30">⏱ {q.timeAllocation}</span>
                </div>
            ...
            {isOpen && (
              <div className="border-t border-white/[0.06]">
                <div className="px-4 py-2.5 bg-white/[0.02] border-b border-white/[0.04] flex items-center justify-between">
                  <p className="text-[10px] font-bold text-blue-300/70 uppercase tracking-widest">Model Answer</p>
...
function MistakesSection({ mistakes }: { mistakes: string[] }) {
  return (
    <div className="space-y-2.5">
      {mistakes.map((m, i) => {
        const parts = m.split("→");
        const wrong = parts[0]?.replace(/^[✗❌\s]+/, "").trim();
        const right = parts[1]?.replace(/^[✓✅\s]+/, "").trim();
        const rawWrong = wrong || m;

        return (
          <div key={i} className="rounded-xl border border-white/[0.06] overflow-hidden">
            <div className="flex items-start gap-3 px-4 py-3 bg-red-500/[0.04]">
              ...
              <p className="text-sm text-red-300/80 leading-relaxed">{rawWrong}</p>
            </div>
            {right && (
        ...
        return (
          <div key={i} className="flex items-start gap-3 rounded-xl border border-white/[0.06] px-3.5 py-3 bg-white/[0.02]">
            ...
            <div className="min-w-0">
              {desc ? (
                <>
                  <p className="text-xs font-bold text-amber-300/80">{key.trim()}</p>
                  <p className="text-xs text-white/50 leading-relaxed mt-0.5">{desc}</p>
                </>
              ) : (
                <p className="text-xs text-white/65 leading-relaxed">{clean}</p>
              )}
            </div>
          </div>
        );
...
function PackageHeader({
  ...
  const isHighRelevance = pkg.examRelevance?.toLowerCase().includes("high");

  return (
    <div
      className="rounded-2xl overflow-hidden border border-white/[0.08]"
      style={{ background: "linear-gradient(135deg, rgba(10,22,40,0.95), rgba(6,13,26,0.98))" }}
    >
      {/* Top strip */}
      <div
        className="px-6 py-3 flex items-center justify-between"
        style={{ background: "linear-gradient(90deg, rgba(251,191,36,0.08), rgba(59,130,246,0.06))", borderBottom: "1px solid rgba(255,255,255,0.05)" }}
      >
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          {revisionId && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-white/[0.05] border border-white/10 text-white/50 hover:text-white/80 hover:border-white/20 transition-all"
            >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-white/[0.05] border border-white/10 text-white/50 hover:text-white/80 hover:border-white/20 disabled:opacity-40 transition-all"
          >
      ...
      <div className="px-6 py-5">
        ...
        {pkg.topicSummary && (
          <p className="text-sm text-white/50 leading-relaxed mb-4 max-w-2xl border-l-2 border-amber-400/30 pl-3">
            {pkg.topicSummary}
          </p>
        )}
        ...
        {pkg.examRelevance && (
          <div className={cn(
            ...
            <Flame className={cn("w-3.5 h-3.5 shrink-0", isHighRelevance ? "text-amber-400" : "text-blue-400")} />
      ...
      <div
        ...
        style={{ borderTop: "1px solid rgba(255,255,255,0.05)", divideBorderColor: "rgba(255,255,255,0.04)" }}
      >
        {[
          ...
          <div key={l} className="flex flex-col items-center py-3.5" style={{ borderRight: "1px solid rgba(255,255,255,0.04)" }}>
            <span
            ...
            <span className="text-[10px] text-white/30 mt-0.5 tracking-wide">{l}</span>
          </div>
        ))}
      </div>
    </div>
...
function LoadingState({ topic }: { topic: string }) {
  const steps = [
    { label: "Analysing ICAI syllabus alignment", icon: Shield },
    { label: "Writing comprehensive revision notes", icon: BookOpen },
    ...
    { label: "Finalising your package", icon: Sparkles },
  ];
  const [step, setStep] = useState(0);

  useState(() => {
    const id = setInterval(() => setStep(p => Math.min(p + 1, steps.length - 1)), 4500);
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center space-y-8">
      ...
      <div className="relative w-20 h-20">
        ...
        <div
          className="absolute inset-0 rounded-full border-2 border-t-amber-400 border-r-transparent border-b-transparent border-l-transparent animate-spin"
      ...
      <p className="text-xs text-white/25">Typically 30–60 seconds · Please don't close this tab</p>
    </div>
  );
}
...
function getSectionText(key: string, data: RevisionPackage): string {
...
function renderSection(key: string, data: RevisionPackage) {
  switch (key) {
    ...
    default: return <TextSection text={getSectionText(key, data)} />;
  }
}
...
export function GenerationForm({ initialTopic = "" }: { initialTopic?: string }) {
  ...
  const doGenerate = useCallback(async (isRegen = false) => {
    ...
    try {
      ...
      const json = await res.json() as GenerateResponse & { error?: string; message?: string; upgradeUrl?: string };
      ...
      setError({ title: "Network Error", message: "Connection failed. Please check your internet and try again." });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-5">
      ...
      >
        {/* Card header */}
        <div
          className="px-6 py-4 flex items-center gap-3 border-b border-white/[0.05]"
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-white/35 uppercase tracking-wider">CA Level</label>
              <select
                value={caLevel}
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-white/35 uppercase tracking-wider">Paper / Subject</label>
              <select
                value={paper}
          ...
          <div className="space-y-2">
            <label className="text-[11px] font-bold text-white/35 uppercase tracking-wider">Topic *</label>
            <input
              value={topic}
          ...
          {/* File Upload */}
          <div className="space-y-1.5">
            <label className="text-[11px] font-bold text-white/35 uppercase tracking-wider">Upload Notes (optional)</label>
            <div
              ...
              className={cn(
                ...
                dragOver ? "border-amber-400/40 bg-amber-400/[0.04]" : file ? "border-emerald-500/30 bg-emerald-500/[0.04]" : "border-white/[0.07] hover:border-white/[0.13] hover:bg-white/[0.02]"
              )}
            >
              {file ? (
                ...
                <>
                  ...
                  <div className="text-center">
                    <p className="text-sm text-white/40">Drop your notes here or click to upload</p>
                    <p className="text-xs text-white/20 mt-0.5">PDF · TXT · DOCX · Max 10MB</p>
                  </div>
                </>
              )}
            </div>
            <input ref={fileRef} type="file" accept=".pdf,.txt,.docx" className="hidden" onChange={e => setFile(e.target.files?.[0] ?? null)} />
          </div>
          ...
          <div className="rounded-xl border border-white/[0.05] overflow-hidden">
            <button
              ...
              className="flex w-full items-center justify-between px-4 py-3 text-sm text-white/40 hover:text-white/60 transition-colors hover:bg-white/[0.02]"
            >
            ...
            {showAdvanced && (
              <div className="border-t border-white/[0.05] p-4 space-y-3">
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-white/30 uppercase tracking-wider">Custom instruction</label>
                  <textarea
                    value={promptTweak}
                    onChange={e => setPromptTweak(e.target.value)}
                    rows={2}
                    placeholder="e.g. Focus more on numericals, add transfer pricing examples..."
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {TWEAK_PRESETS.map(p => (
                      <button
                        key={p.label}
                        onClick={() => setPromptTweak(p.value)}
          ...
          {!loading && (
            <p className="text-center text-xs text-white/20">
              ⚡ 30–60 seconds · ICAI 2026 aligned · All 8 sections
            </p>
          )}
        </div>
      </div>
        ...
        <div ref={resultRef} className="space-y-3">
          ...
          {SECTIONS.map(sec => (
            <SectionShell
              ...
              copyText={getSectionText(sec.key, result.data)}
              sectionKey={sec.key}
            >
          ...
          {result.revisionId && (
            <div className="fixed bottom-6 right-6 z-50">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-2 rounded-2xl bg-amber-400 hover:bg-amber-300 px-5 py-3 text-sm font-black text-black shadow-2xl shadow-amber-400/30 transition-all hover:scale-105"
              >
